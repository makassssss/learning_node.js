...
function* addDepartment(action) {
	...
	const { name } = action;
	const ApiCall = () =>
		axios.post('http://localhost:5000/api/add-department', { name }, config)
		.then(res => res.data);
...
function* addEmployee(action) {
	const config = configWithAuthHeader();
    const { departmentId, name, email, birthday, salary } = action;
	const ApiCall = () =>
    ...
    success
		? yield put({ type: ADD_EMPLOYEE_DONE, departmentId, id, name, email, birthday, salary })
...
function* editEmployee(action) {
	const config = configWithAuthHeader();
    const { departmentId, id, name, email, birthday, salary } = action;
	const ApiCall = () =>
    ...
    const { success, err } = response;
    success
		? yield put({ type: EDIT_EMPLOYEE_DONE, departmentId, id, name, email, birthday, salary })
